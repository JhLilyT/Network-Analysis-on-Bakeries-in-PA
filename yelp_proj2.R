...
pa_bakeries = read.csv('yelp_bakeries_pa_updated.csv')
...
#Create adjacency matrix and g object
...
V(g)$business_accepts_card = as.character(pa_bakeries$BusinessAcceptsCreditCards[match(V(g)$name, pa_bakeries$business_id)])
V(g)$bike_parking = as.character(pa_bakeries$BikeParking[match(V(g)$name, pa_bakeries$business_id)])
V(g)$delivery = as.character(pa_bakeries$RestaurantsDelivery[match(V(g)$name, pa_bakeries$business_id)])
V(g)$takeout = as.character(pa_bakeries$RestaurantsTakeOut[match(V(g)$name, pa_bakeries$business_id)])
V(g)$outdoor_seating = as.character(pa_bakeries$OutdoorSeating[match(V(g)$name, pa_bakeries$business_id)])
V(g)$parking_garage = as.character(pa_bakeries$parking_garage[match(V(g)$name, pa_bakeries$business_id)])
V(g)$parking_street = as.character(pa_bakeries$parking_street[match(V(g)$name, pa_bakeries$business_id)])
V(g)$parking_validated = as.character(pa_bakeries$parking_validated[match(V(g)$name, pa_bakeries$business_id)])
V(g)$parking_lot = as.character(pa_bakeries$parking_lot[match(V(g)$name, pa_bakeries$business_id)])
V(g)$parking_valet = as.character(pa_bakeries$parking_valet[match(V(g)$name, pa_bakeries$business_id)])
V(g)$name = as.character(pa_bakeries$name[match(V(g)$name, pa_bakeries$business_id)])

#adj = get.adjacency(g, attr='weight')
#adj = as.data.frame(as.matrix(adj))
